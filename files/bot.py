...
async def button_handler(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    # ══════════════════════════════════════════════════════
    #  USER PROFILE (فقط اطلاعات حساب کاربری)
    # ══════════════════════════════════════════════════════
    ...
    # ══════════════════════════════════════════════════════
    #  MY REFERRAL LINK (فقط لینک دعوت — جدا از حساب کاربری)
    # ══════════════════════════════════════════════════════
    ...
    # ── Admin: customers ───────────────────────────────────────────────────────
        ...
        users = c.execute(
            ...
            "FROM users WHERE is_verified=1 ORDER BY id DESC LIMIT 60"
        ).fetchall()
        c.close()
        if not users:
            await q.edit_message_text("👥 هیچ مشتری ثبت‌نامی وجود ندارد.",
                                      reply_markup=InlineKeyboardMarkup([
                                          [InlineKeyboardButton("🔍 جستجوی مشتری", callback_data="admin_user_search")],
        ...
        rows.append([InlineKeyboardButton("🔍 جستجوی مشتری", callback_data="admin_user_search")])
        rows.append([InlineKeyboardButton("◀️ بازگشت", callback_data="admin_panel")])
        await q.edit_message_text(f"👥 *لیست مشتریان ({len(users)} نفر):*",
                                  parse_mode=ParseMode.MARKDOWN,
                                  reply_markup=InlineKeyboardMarkup(rows))

    # ── Admin: search customer (دکمه جستجوی مشتری) ─────────────────────────────
    elif data == "admin_user_search":
        if uid != ADMIN_CHAT_ID: return
        ctx.user_data["admin_action"] = "search_user_query"
        await q.edit_message_text(
            "🔍 *جستجوی مشتری*\n\n"
            "نام، شماره تماس، ایمیل، یوزرنیم یا Telegram ID را وارد کنید:",
            parse_mode=ParseMode.MARKDOWN, reply_markup=back_home())
        ...
        await q.edit_message_text(
            f"👤 *{u_['full_name'] or '—'}*\n"
            f"🆔 شناسه تلگرام: `{u_['telegram_id']}`\n"
            f"🔗 یوزرنیم: @{u_['telegram_username'] or '—'}\n"
            f"📧 {u_['email'] or '—'}\n📱 {u_['phone'] or '—'}\n"
    ...
    # ── Admin: search order (دکمه جستجوی سفارش) ────────────────────────────────
    elif data == "admin_order_search":
        if uid != ADMIN_CHAT_ID: return
        ctx.user_data["admin_action"] = "search_order_query"
        await q.edit_message_text(
            "🔍 *جستجوی سفارش*\n\n"
            "شماره سفارش، نام مشتری، شماره تماس، یوزرنیم یا نام محصول را وارد کنید:",
            parse_mode=ParseMode.MARKDOWN, reply_markup=back_home())
    ...
    if not orders:
        await q.edit_message_text("📦 هیچ سفارشی یافت نشد.",
                                  reply_markup=InlineKeyboardMarkup([tabs[:2], tabs[2:],
                                      [InlineKeyboardButton("🔍 جستجوی سفارش", callback_data="admin_order_search")],
                                      [InlineKeyboardButton("◀️ بازگشت", callback_data="admin_panel")]])); return
    smap = {"pending": "⏳", "approved": "✅", "rejected": "❌"}
    rows = [[InlineKeyboardButton(
        f"{smap.get(o['status'],'❓')} #{o['id']} | {o['full_name']} | {o['pname']} ×{o['quantity']}",
        callback_data=f"admin_order_{o['id']}")] for o in orders]
    rows.append([InlineKeyboardButton("🔍 جستجوی سفارش",  callback_data="admin_order_search")])
...
async def message_handler(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    # ── Admin: customer search query processing ────────────────────────────────
    if ctx.user_data.get("admin_action") == "search_user_query":
        ...
        results = c.execute(
            "SELECT telegram_id, COALESCE(full_name,'—') as full_name, "
            "COALESCE(telegram_username,'—') as telegram_username "
            "FROM users WHERE is_verified=1 AND ("
            "CAST(telegram_id AS TEXT) LIKE ? OR full_name LIKE ? OR phone LIKE ? "
            "OR email LIKE ? OR telegram_username LIKE ?) LIMIT 30",
        ...
        if not results:
            await update.message.reply_text(
                ...
                reply_markup=InlineKeyboardMarkup([
                    [InlineKeyboardButton("🔍 جستجوی دوباره",     callback_data="admin_user_search")],
                    [InlineKeyboardButton("◀️ بازگشت به لیست",   callback_data="admin_users")],
                ])); return
        rows = [[InlineKeyboardButton(
            f"👤 {r['full_name']} | 🆔 {r['telegram_id']}",
            callback_data=f"admin_user_{r['telegram_id']}")] for r in results]
        rows.append([InlineKeyboardButton("🔍 جستجوی دوباره",   callback_data="admin_user_search")])
        rows.append([InlineKeyboardButton("◀️ بازگشت به لیست", callback_data="admin_users")])
        await update.message.reply_text(
            f"🔍 *نتایج جستجو ({len(results)} مورد):*",
            parse_mode=ParseMode.MARKDOWN, reply_markup=InlineKeyboardMarkup(rows)); return

    # ── Admin: order search query processing ────────────────────────────────────
    if ctx.user_data.get("admin_action") == "search_order_query":
        ctx.user_data["admin_action"] = None
        like = f"%{text.strip()}%"
        c = db()
        results = c.execute(
            ...
            "WHERE CAST(o.id AS TEXT) LIKE ? OR u.full_name LIKE ? OR u.phone LIKE ? "
            "OR u.telegram_username LIKE ? OR p.name LIKE ? OR CAST(o.user_id AS TEXT) LIKE ? "
        ...
        if not results:
            await update.message.reply_text(
                ...
                reply_markup=InlineKeyboardMarkup([
                    [InlineKeyboardButton("🔍 جستجوی دوباره",   callback_data="admin_order_search")],
                    [InlineKeyboardButton("◀️ بازگشت به لیست", callback_data="admin_orders")],
                ])); return
        smap = {"pending": "⏳", "approved": "✅", "rejected": "❌"}
        rows = [[InlineKeyboardButton(
            f"{smap.get(r['status'],'❓')} #{r['id']} | {r['full_name']} | {r['pname']} ×{r['quantity']}",
            callback_data=f"admin_order_{r['id']}")] for r in results]
        rows.append([InlineKeyboardButton("🔍 جستجوی دوباره",   callback_data="admin_order_search")])
        rows.append([InlineKeyboardButton("◀️ بازگشت به لیست", callback_data="admin_orders")])
        await update.message.reply_text(
            f"🔍 *نتایج جستجو ({len(results)} مورد):*",
            parse_mode=ParseMode.MARKDOWN, reply_markup=InlineKeyboardMarkup(rows)); return
